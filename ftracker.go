...
func RunningSpentCalories(action int, weight, duration float64) float64 {
    ...
    return ((runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift) * weight / mInKM 
    * duration * minInH)
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
    // ваш код здесь
    averageSpeedMsec := meanSpeed(action, duration) / kmhInMsec
  
    return ((walkingCaloriesWeightMultiplier * weight + (math.Pow(averageSpeedMsec, 2) / height)
